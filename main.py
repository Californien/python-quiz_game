...
if is_pycharm_console():
    print(
        Fore.RED + Style.BRIGHT + 'Beachte, dass zur korrekten Kompatibilität `main.py` in einem Terminal ausgeführt werden muss!')
    print(Fore.RED + Style.BRIGHT + 'Beende jetzt das Programm und starte es in einem Terminal neu!')
    print(Fore.RED + Style.DIM + 'Press `ALT + F12`  >  python main.py\n' + Style.RESET_ALL)
    print(Fore.CYAN + Style.DIM + f'os.environ: {str(os.environ)}')
...
if not is_pycharm_console():
    ...
    def runSingleplayerCfg():
        ...
        time.sleep(0.5)
        print(
            f'{Fore.LIGHTRED_EX + Style.BRIGHT + '? '} {reset + Style.BRIGHT + 'Wie viele Fragen möchtest du beantworten?'}')
    ...
    def runMultiplayerCfg(count):
        ...
        time.sleep(0.5)
        print(
            f'{Fore.LIGHTRED_EX + Style.BRIGHT + '? '} {reset + Style.BRIGHT + 'Wie viele Fragen möchtet ihr beantworten?'}')
        print(
            f'{Fore.LIGHTBLUE_EX + Style.NORMAL + "\nDrücke '1' für 12 Fragen, '2' für 24 Fragen, '3' für 40 Fragen."}\n')
        qCount = 0
        while True:
            if keyboard.is_pressed('1'):
                qCount = 12
        ...
        for i in range(1, (count + 1)):
            ...
            names.append(name)
            print(
                Fore.LIGHTGREEN_EX + Style.BRIGHT + '✓ ' + Fore.LIGHTWHITE_EX + Style.NORMAL + f'Okay {name}, du bist Spieler {i} und erhälst den Buzzer {binds[i - 1].upper()}')
            time.sleep(2)
        print(
        ...
        while True:
            try:
                ...
                if wrong_answers == 0:
                    ...
                    print(
                        Fore.RED + 'Wer sich vor dem Quiz schon so anstellt, kann im Quiz selbst nicht weit kommen...')
                    wrong_answers = wrong_answers + 1
                elif wrong_answers > 1:
